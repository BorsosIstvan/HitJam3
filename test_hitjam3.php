...
<body>
    ...
    <div class="card">
        ...
        <button id="btn-next">▶️ Speel willekeurig nummer</button>
    ...
    <!-- KAART 2: Antwoord Simulatie -->
    <div class="card" id="answering-card" style="display:none;">
        <h3>2. Tijdlijn Gok Simuleren</h3>
        <p>Stel dat je al een liedje uit <strong>2010</strong> op je scherm hebt staan. Waar plaats je dit nieuwe nummer?</p>
        
        <div class="timeline-mock">
            <!-- We sturen fictieve ID's mee om de PHP-werking te testen -->
            <button onclick="stuurAntwoord(0, 10)">⬅️ VÓÓR 2010 (Ouder)</button>
            <button onclick="stuurAntwoord(10, 0)">➡️ NÁ 2010 (Nieuwer)</button>
        </div>
    </div>

    <!-- Feedback meldingen -->
    <div id="feedback-box" class="alert"></div>

    <script>
        // Globale variabelen voor de JavaScript-motor
        let audioPlayer = null;
        let huidigSongId = null;

        // Knoppen koppelen aan functies
        document.getElementById('btn-next').addEventListener('click', haalNummerOp);
        document.getElementById('btn-fallback').addEventListener('click', haalNummerOp);
        ...
        function stuurAntwoord(songBeforeId, songAfterId) {
            if (!huidigSongId) return;

            // In een echte game is player_id dynamisch, voor nu testen we met speler 1
            let formData = new FormData();
            formData.append('player_id', 1);
            formData.append('current_song_id', huidigSongId);
            formData.append('song_before_id', songBeforeId);
            formData.append('song_after_id', songAfterId);

            // Stuur de gok naar de PHP-checker op de Pi [1]
            fetch('hj3_check_answer.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
                    if (data.result === 'correct') {
                        toonFeedback(`🎉 Goed zo! Het nummer kwam uit ${data.year}. Toegevoegd aan je tijdlijn!`, 'success');
                    } else {
                        toonFeedback(`😢 Helaas! Het nummer kwam uit ${data.year}. Verkeerde plek.`, 'error');
                    }
                    // Verberg de antwoordknoppen tot het volgende nummer geladen wordt
                    document.getElementById('answering-card').style.display = 'none';
                } else {
                    toonFeedback("Fout bij controle: " + data.message, 'error');
                }
            });
        }
